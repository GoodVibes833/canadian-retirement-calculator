...
- spouse-aware survivor logic now includes a baseline spousal asset rollover, defined-benefit survivor continuation, partial CPP / QPP survivor-pension support, and a user-overridable survivor spending path that defaults to 72% of couple spending, but full combined-benefit math, estate-tax effects, and probate handling remain incomplete,
