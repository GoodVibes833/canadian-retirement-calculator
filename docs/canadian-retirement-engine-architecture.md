...
- spouse-aware survivor logic now includes a baseline spousal asset rollover, defined-benefit survivor continuation, partial CPP / QPP survivor-pension support, a user-overridable survivor spending path that defaults to 72% of couple spending, and a mid-year death heuristic that prorates recurring income / mandatory withdrawals in couple death years and adds a half-year survivor public pension baseline. QPP age-65-plus combined-benefit cases now use a warning-heavy approximation, and summary output now includes a projection-end after-tax estate proxy that applies terminal tax to remaining registered balances, uses net capital losses against terminal gains and then other income on a final-return baseline, and approximates ON / BC / AB probate-style estate administration costs. Full CRA final-return replication, Quebec will-form probate detail, and DC pension death treatment remain incomplete,
...
4. Add fuller survivor modeling including actual CRA final-return handling, beneficiary-designation-aware estate flow, Quebec will-form probate detail, and more exact CPP / QPP combined-benefit behavior below age 65 and at form-level detail.
